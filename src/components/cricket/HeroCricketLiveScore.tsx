...
export const HeroCricketLiveScore: React.FC = () => {
  ...
  const handleOpenDetailScoreboard = (matchId?: string) => {
    const targetMatch = (matchId ? liveMatches.find(m => m.id === matchId) : null) 
      || activeMatch 
  ...
  // Synchronize ONLY live ongoing matches
  useEffect(() => {
    purgeCachedAIMatches();
    ...
    const loadInitialLocal = () => {
      try {
        ...
        if (activeLocal && !isMatchDeleted(activeLocal.id) && !isDemoOrAIMatch(activeLocal) && activeLocal.status === 'live') {
          activeList.push(activeLocal);
        }

        registry.forEach(m => {
          if (!isMatchDeleted(m.id) && !isDemoOrAIMatch(m) && m.status === 'live' && !activeList.some(a => a.id === m.id)) {
            activeList.push(m);
          }
        });
        ...
        console.warn('[HeroCricketLiveScore] Local initial load error:', err);
      }
    };
  ...
  // Derived scoring calculations for live match
  const matchDetails = useMemo(() => {
    if (!activeMatch || activeMatch.status !== 'live') return null;

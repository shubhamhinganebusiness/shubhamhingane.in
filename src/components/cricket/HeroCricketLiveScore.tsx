...
export const HeroCricketLiveScore: React.FC = () => {
  ...
  // If NO match is currently live, but a recently completed match exists:
  // Display the Official Match Result Card
  const latestCompleted = recentCompletedMatches[0];
  if (liveMatches.length === 0 && latestCompleted) {
    const inn1 = latestCompleted.innings1;
    const inn2 = latestCompleted.innings2;
    const teamAScore = inn1 ? `${inn1.runs}/${inn1.wickets}` : '0/0';
    const teamAOvers = inn1 ? `${Math.floor(inn1.ballsBowled / 6)}.${inn1.ballsBowled % 6}` : '0.0';
    const teamBScore = inn2 ? `${inn2.runs}/${inn2.wickets}` : (latestCompleted.status === 'completed' ? 'Yet to bat' : '0/0');
    const teamBOvers = inn2 ? `${Math.floor(inn2.ballsBowled / 6)}.${inn2.ballsBowled % 6}` : '0.0';
    const winnerText = latestCompleted.winner === 'Tie'
      ? 'Match Ended in a Thrilling Tie!'
      : `${latestCompleted.winner} ${latestCompleted.winReason || 'won the match'}`;
    ...
    return (
      <motion.div
        ...
        className="w-full max-w-xl mb-6 p-4 sm:p-5 rounded-[2rem] bg-gradient-to-br from-slate-900 via-zinc-900 to-slate-950 border border-amber-500/40 text-white backdrop-blur-xl shadow-2xl shadow-amber-950/20 select-none relative overflow-hidden"
      >
        ...
        <div className="flex flex-col gap-3 relative z-10">
          ...
          <div className="flex items-center justify-between gap-2 border-b border-white/10 pb-2.5">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-500/20 text-amber-300 border border-amber-500/30">
                ...
                Match Finished • Official Result
              </span>
              {latestCompleted.oversLimit && (
                <span className="text-[10px] font-bold text-slate-400 uppercase">
                  {latestCompleted.oversLimit} Ov Match
                </span>
              )}
            </div>
            {recentCompletedMatches.length > 1 && (
              <span className="text-[10px] font-bold text-slate-400">
                Latest of {recentCompletedMatches.length} matches
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 py-1">
            <div className="p-3 rounded-2xl bg-white/[0.04] border border-white/5 flex flex-col justify-between">
              <span className="text-xs font-black uppercase text-slate-300 truncate mb-1">
                {latestCompleted.teamA}
              </span>
          ...
          <div className="p-2.5 rounded-xl bg-gradient-to-r from-amber-500/15 via-emerald-500/15 to-transparent border border-amber-500/30 flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              id="hero-view-completed-result-btn"
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                handleOpenDetailScoreboard(latestCompleted.id);
              }}
              className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-slate-950 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-md shadow-amber-500/20 cursor-pointer border-none"
              title="View full final scorecard & match summary in the Spectator Scoreboard Section"
            >
              <span>View Full Scorecard & Result</span>
              <ArrowRight size={14} />
            </button>
            ...
            {isScoreManager && (
              <button
                ...
                className="px-3.5 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-emerald-300 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1 transition-all cursor-pointer border border-emerald-500/30"
  ...
  // If NO match is currently live and no completed match:
  // Modern, attractive broadcast match center card
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className="w-full max-w-xl mb-6 p-4.5 rounded-[2rem] bg-gradient-to-r from-slate-900/90 via-zinc-900/90 to-slate-950/95 border border-emerald-500/30 text-white backdrop-blur-xl shadow-xl shadow-emerald-950/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 group hover:border-emerald-500/50 transition-all select-none"
    >
      <div className="flex items-center gap-3.5 min-w-0">
        <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-emerald-500/20 to-teal-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shrink-0 shadow-inner">
          <Trophy size={20} className="group-hover:scale-110 transition-transform" />
        </div>
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-xs sm:text-sm font-black uppercase tracking-wider text-white truncate">
              GullyScore Live Arena
            </span>
          ...
          <span className="text-xs text-slate-400 truncate">
            Live ball-by-ball updates, commentary & tournament ledger
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
        <button
          ...
          className="flex-1 sm:flex-none px-4.5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shadow-md shadow-emerald-500/20 cursor-pointer border-none hover:scale-105 active:scale-95"
          title="Open Spectator Scoreboard Section"
        >
          <span>Spectator Scoreboard</span>
          <ArrowRight size={13} />
        </button>

        {/* Only authenticated scoreboard manager can access manager hub */}
        {isScoreManager && (
          <button
            type="button"
            onClick={(e) => {
            ...
            className="px-3 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-emerald-300 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1 transition-all cursor-pointer border border-emerald-500/30"
            title="Open GullyScore Cricket Scorer Console (Score Managers Only)"
          >

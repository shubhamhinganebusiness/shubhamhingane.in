...
export const StarTVScorebug: React.FC<StarTVScorebugProps> = ({
  ...
  // ---------------------------------------------------------------------------
  // TOURNAMENT PRIZE MONEY (ABOVE SCOREBUG DISPLAY & CONTINUOUS ROTATION)
  // ---------------------------------------------------------------------------
  const [internalPrizes, setInternalPrizes] = useState<TournamentPrize[]>(() => {
    if (tournamentPrizes && tournamentPrizes.length > 0) return tournamentPrizes;
    return getTournamentPrizes(matchId);
  });
  const [activePrizeIndex, setActivePrizeIndex] = useState(0);
  const [isPrizeBannerPaused, setIsPrizeBannerPaused] = useState(false);

  useEffect(() => {
    if (tournamentPrizes && tournamentPrizes.length > 0) {
      setInternalPrizes(tournamentPrizes);
      return;
    }
    ...
    return () => unsub();
  }, [tournamentPrizes, matchId]);

  const activePrizes = useMemo(() => {
    const list = tournamentPrizes && tournamentPrizes.length > 0 ? tournamentPrizes : internalPrizes;
    return getValidActivePrizes(list);
  }, [tournamentPrizes, internalPrizes]);

  // Continuous auto-rotation for the Prize Banner above scorebug (every 4.5s)
  useEffect(() => {
    if (isPrizeBannerPaused || activePrizes.length <= 1) return;
    const timer = setInterval(() => {
      setActivePrizeIndex((prev) => (prev + 1) % activePrizes.length);
    }, 4500);
    return () => clearInterval(timer);
  }, [isPrizeBannerPaused, activePrizes.length]);

  const safePrizeIndex = activePrizes.length > 0 ? activePrizeIndex % activePrizes.length : 0;
  const currentPrize = activePrizes[safePrizeIndex] || activePrizes[0];
  ...
  const tickerSlides = useMemo(() => {
    return [
      ...
      // Appended Tournament Prizes Slide in Ticker (Active when score manager adds details)
      ...(activePrizes.length > 0 ? [{
        id: 'tournament_prizes_ticker',
        category: 'TOURNAMENT CASH PRIZES',
        categoryColor: 'text-amber-400',
        content: (
          <span className="text-white inline-flex items-center gap-2 justify-center flex-wrap">
            <Trophy size={13} className="text-amber-400 shrink-0 inline-block animate-pulse" />
            <span className="text-amber-400 font-black">CASH AWARDS:</span>
              ...
              <React.Fragment key={p.id || idx}>
                ...
                {p.personPhoto ? (
                  <img 
                    src={p.personPhoto} 
                    alt={p.personName} 
                    className="w-4 h-4 rounded-full object-cover border border-amber-400 inline-block shrink-0 shadow-xs"
                    referrerPolicy="no-referrer"
                  />
                ) : null}
                <span className="text-amber-300 font-extrabold">{p.title}:</span>
                <strong className="text-white font-mono bg-amber-500/20 px-1 py-0.5 rounded border border-amber-500/30">
                  {p.currency || '₹'}{p.amount}
                </strong>
                {p.personName && (
                  <span className="text-slate-300 text-[10.5px]">
                    (by <span className="text-yellow-200 font-bold">{p.personName}</span>)
                  </span>
                )}
              </React.Fragment>
            ))}
  ...
  return (
    ...
    >
      ...
      {/* =========================================================================
          ...
          Displays continuously above the TV Scorebug when prize details are added.
          If score manager has not added details, it remains completely hidden.
          ========================================================================= */}
      {showPrizeMoneyBanner && activePrizes.length > 0 && currentPrize && (
        <div 
          id="star-tv-above-scorebug-prize-banner"
          onMouseEnter={() => setIsPrizeBannerPaused(true)}
          onMouseLeave={() => setIsPrizeBannerPaused(false)}
          className="relative w-full overflow-hidden bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border-t-2 border-b border-amber-400/70 py-1.5 px-3 sm:px-6 shadow-2xl backdrop-blur-xl z-30 transition-all flex items-center justify-between gap-3 font-sans"
        >
          {/* Subtle Golden Glow Shimmer Behind */}
          <div className="absolute inset-0 bg-gradient-to-r from-amber-500/10 via-yellow-400/5 to-amber-500/10 pointer-events-none" />

          {/* Left Award Category Badge with pulsing indicator */}
          <div className="flex items-center gap-2 shrink-0 z-10">
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-gradient-to-r from-amber-500/25 to-yellow-500/20 border border-amber-400/40 text-amber-300 shadow-sm">
              <Trophy size={13} className="text-yellow-400 animate-pulse" />
              <span className="text-[9px] sm:text-[10.5px] font-black uppercase tracking-wider">
                {currentPrize.title || 'TOURNAMENT AWARD'}
              </span>
            </div>
          </div>

          {/* Center Carousel: Sponsor Person Photo, Name & Title */}
          <div className="flex-1 mx-2 overflow-hidden flex items-center justify-center text-center z-10">
            <AnimatePresence mode="wait">
              <motion.div
                key={`${currentPrize.id}-${safePrizeIndex}`}
              ...
              >
                ...
                {/* Text Details: Sponsor Person Name & Designation */}
                <div className="text-left flex flex-col sm:flex-row sm:items-center sm:gap-2 leading-tight">
                  ...
                  <div className="flex items-center gap-1.5">
                    <strong className="text-xs sm:text-sm font-black uppercase tracking-wide text-white drop-shadow-sm">
                      {currentPrize.personName || 'Tournament Patron'}
                    </strong>
                    {currentPrize.personDesignation && (
                      <span className="text-[9px] sm:text-[10px] font-bold text-amber-300/90 bg-amber-500/15 px-1.5 py-0.2 rounded border border-amber-500/30">
                        {currentPrize.personDesignation}
                      </span>
                    )}
                  </div>
          ...
          {/* Right Prize Money Badge & Controls */}
          <div className="flex items-center gap-2 shrink-0 z-10">
            {/* Glowing Golden Cash Prize Pill */}
            <div className="px-2.5 sm:px-3.5 py-1 bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-400 text-slate-950 font-black rounded-lg sm:rounded-xl text-[11px] sm:text-xs tracking-tight shadow-md flex items-center gap-1">
              ...
              <span className="font-mono font-black text-xs sm:text-sm">
                {currentPrize.currency || '₹'} {currentPrize.amount}
              </span>
            </div>

            {/* Pagination Controls if > 1 Prize */}
                  ...
                  {activePrizes.map((p, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all cursor-pointer border-none ${
                        safePrizeIndex === i ? 'bg-amber-400 w-3' : 'bg-white/25 hover:bg-white/40 w-1.5'
                      }`}
                      title={p.title}
                    />
                  ))}

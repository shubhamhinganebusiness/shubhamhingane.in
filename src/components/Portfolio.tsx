...
import { 
  ExternalLink, Plus, ArrowRight, ChevronLeft, ChevronRight, 
  Sparkles, Layers, ArrowUpRight, LayoutGrid, SlidersHorizontal, 
  Search, Eye, CheckCircle2, ShieldCheck, Key, Cpu, X
} from 'lucide-react';
...
export const Portfolio: React.FC = () => {
    ...
    return projects.map((project: any) => {
      if (project.id === 'dairy-management') {
        return { 
          ...
          techStack: ['React 19', 'Firebase', 'Tailwind CSS', 'Multi-tenant'],
      ...
      if (project.id === 'cricket-scoreboard') {
        return {
          ...
          techStack: ['React', 'Firebase RTDB', 'OBS Overlay', 'Web Audio API'],
  ...
  // Unique list of all tech stack items for quick filter pills
  ...
  // Multi-attribute filtering (category + tech stack + keyword search)
  const filteredProjects = useMemo(() => {
    return allProjects.filter((project: any) => {
      ...
      const matchesSearch = !query || 
        ...
        (project.desc && project.desc.toLowerCase().includes(query)) ||
        (project.badge && project.badge.toLowerCase().includes(query)) ||
        (project.techStack && project.techStack.some((t: string) => t.toLowerCase().includes(query)));
      return matchesCategory && matchesTech && matchesSearch;
    });
  }, [allProjects, filter, selectedTech, searchQuery]);
  ...
  return (
    <section 
      ...
      className={`${
        viewMode === 'horizontal' 
          ? 'min-h-screen lg:h-screen lg:max-h-screen flex flex-col justify-between' 
          : 'py-12 md:py-16'
      } py-4 sm:py-6 bg-main-bg relative overflow-hidden transition-colors select-none focus:outline-none`}
    >
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 shrink-0">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-3 sm:mb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="hidden sm:inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                <CheckCircle2 size={12} /> 12+ Live Production Deployments
              </span>
            </div>
          ...
          {/* Search, View Mode Toggle & Desktop Nav Controls */}
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="relative w-full sm:w-56">
              ...
              <input
                ...
                placeholder="Search tech, live app..."
                className="w-full pl-8 pr-7 py-1.5 rounded-xl bg-surface border border-gray-200/80 dark:border-zinc-800 text-xs font-semibold text-main-text placeholder-gray-400 focus:outline-none focus:border-primary transition-colors"
              />
              {searchQuery && (
                <button
            ...
            <div className="flex items-center p-1 rounded-xl bg-surface border border-gray-200/80 dark:border-zinc-800 shadow-xs">
              <button
                type="button"
                onClick={() => setViewMode('horizontal')}
                title="Horizontal Flow Showcase"
              ...
              >
                ...
                <span className="hidden sm:inline text-[10px] uppercase font-black">Slider</span>
              </button>
              <button
                type="button"
              ...
              >
                <LayoutGrid size={14} />
                <span className="hidden sm:inline text-[10px] uppercase font-black">Grid</span>
              </button>
            </div>

            {/* Horizontal Arrow Buttons (only in horizontal mode) */}
            {viewMode === 'horizontal' && (
              <div className="hidden sm:flex items-center gap-1.5">
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-surface border border-gray-200/70 dark:border-zinc-800 text-xs font-mono font-bold text-gray-600 dark:text-gray-300">
                ...
                <button
                  ...
                  className={`p-2 rounded-xl border transition-all ${
                    canScrollLeft
                      ? 'bg-surface text-main-text border-gray-200 dark:border-zinc-800 hover:border-primary hover:text-primary active:scale-95 shadow-sm cursor-pointer'
        ...
        {/* Filters Row (Category Tabs + Quick Tech Pills) */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="hidden lg:flex items-center gap-1.5 overflow-x-auto custom-scrollbar">
            ...
            {allTechStacks.map((tech) => (
              <button
                ...
                className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-colors cursor-pointer whitespace-nowrap ${
                  selectedTech === tech
                    ? 'bg-primary text-white'
                    : 'bg-surface border border-gray-200 dark:border-zinc-800 text-gray-600 dark:text-gray-400 hover:border-primary hover:text-primary'
                }`}
              >
                {tech}
              </button>
      ...
      {/* MIDDLE: PROJECT SHOWCASE (HORIZONTAL OR GRID) */}
      {viewMode === 'horizontal' ? (
        <div className="flex-1 min-h-0 flex items-center w-full my-auto py-2 sm:py-3 relative group/slider">
          ...
          <div className="absolute left-2 sm:left-4 z-20 hidden md:block pointer-events-none opacity-0 group-hover/slider:opacity-100 transition-opacity">
            <button
              ...
              className={`p-3 rounded-2xl bg-surface/90 backdrop-blur-md shadow-xl border border-gray-200 dark:border-zinc-800 text-main-text hover:text-primary hover:border-primary pointer-events-auto transition-all transform active:scale-95 cursor-pointer ${
          ...
          >
            {filteredProjects.map((project: any, idx: number) => (
              <motion.div
                ...
                className="group/card flex-shrink-0 w-[290px] sm:w-[380px] md:w-[440px] lg:w-[460px] xl:w-[480px] h-[390px] sm:h-[420px] lg:h-[440px] bg-surface rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 border border-gray-200/80 dark:border-zinc-800 snap-center relative flex flex-col justify-between"
              >
                ...
                <div className="relative h-40 sm:h-44 lg:h-48 shrink-0 overflow-hidden bg-gray-100 dark:bg-zinc-800">
                  ...
                  {/* Interactive Quick Inspect Hover Action */}
                  <div className="absolute inset-0 bg-primary/25 backdrop-blur-[2px] opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2.5 z-20">
                    <button 
                      type="button"
                      onClick={() => setInspectProject(project)}
                      className="p-3 bg-white text-primary rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-transform flex items-center gap-1.5 font-bold text-xs uppercase tracking-wider cursor-pointer"
                    >
                      <Eye size={16} />
                    ...
                    <Link 
                      ...
                      className="p-3 bg-surface text-main-text rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-transform flex items-center gap-1.5 font-bold text-xs uppercase tracking-wider"
                    >
                      <Plus size={16} />
                      <span>Explore</span>
                    </Link>
                  ...
                  <div className="pt-3 border-t border-gray-100 dark:border-zinc-800/80 flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setInspectProject(project)}
                        className="p-1.5 text-gray-400 hover:text-primary rounded-xl transition-colors cursor-pointer"
                        title="Quick Inspect"
                      >
                        <Eye size={16} />
                      </button>
          ...
          {filteredProjects.length === 0 && (
            <div className="w-full text-center py-12">
              <p className="text-gray-500 text-sm">No projects match the current filters.</p>
            </div>
          )}
        </div>
      ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {filteredProjects.map((project: any, idx: number) => (
              <motion.div
                ...
                className="bg-surface rounded-3xl overflow-hidden card-shadow border border-gray-200/80 dark:border-zinc-800 flex flex-col justify-between group hover:border-primary/50 transition-all duration-300"
              >
                {/* Visual Thumbnail */}
                <div className="relative h-48 overflow-hidden bg-gray-100 dark:bg-zinc-800">
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                    <span className="px-2.5 py-1 bg-black/60 backdrop-blur-md rounded-lg text-[10px] font-black text-white uppercase tracking-wider">
                      {project.badge || project.category}
                    </span>
                    {project.isPaidSystem && (
                      <span className="px-2.5 py-1 bg-primary text-white rounded-lg text-[9px] font-black uppercase tracking-wider">
                        SaaS Suite
                      </span>
                    )}
                  </div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {project.techStack?.slice(0, 4).map((tech: string) => (
                        <span 
                          key={tech} 
                          className="px-2 py-0.5 bg-gray-100 dark:bg-zinc-800 rounded-md text-[10px] font-semibold text-gray-600 dark:text-gray-300"
                        >
                          {tech}
                        </span>
                      ))}
                  ...
                  <div className="pt-3 border-t border-gray-100 dark:border-zinc-800 flex items-center justify-between">
                    <button
                      type="button"
                      onClick={() => setInspectProject(project)}
                      className="px-3 py-1.5 rounded-xl border border-gray-200 dark:border-zinc-700 hover:border-primary text-main-text font-bold text-xs transition-colors flex items-center gap-1.5 cursor-pointer"
                    >
                      ...
                      {project.demoUrl && (
                        project.demoUrl.startsWith('http') ? (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-3 py-1.5 bg-primary text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:brightness-110 flex items-center gap-1 shadow-sm"
                          >
                            <span>Live App</span>
                            <ArrowUpRight size={13} />
                          </a>
          ...
          {filteredProjects.length === 0 && (
            <div className="w-full text-center py-16">
              <p className="text-gray-500 text-sm">No projects match your search or filter criteria.</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 shrink-0">
        <div className="flex items-center justify-between gap-4 pt-2 border-t border-gray-200/60 dark:border-zinc-800">
          ...
          <div className="hidden md:flex items-center gap-2 text-[11px] text-gray-400 font-mono">
            <span>{viewMode === 'horizontal' ? 'Use ← → keys or swipe' : 'Click Inspect to view architecture'}</span>
          </div>
        </div>
      </div>

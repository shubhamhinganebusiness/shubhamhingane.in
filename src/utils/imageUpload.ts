    ...
    try {
      ...
      return await new Promise<UploadResult>((resolve) => {
        uploadTask.on(
          ...
          (error) => {
            console.warn('[Firebase Storage] Direct upload error, checking backend route:', error?.message || error);
            // Fall through to backend route below
            resolve(uploadImageViaBackend(uploadBlob, {
              folder,
              entityId,
              maxWidth,
              maxHeight,
          ...
          async () => {
            try {
              ...
              // Try backend route if download url fails
              resolve(uploadImageViaBackend(uploadBlob, {
                folder,
                entityId,
                maxWidth,
